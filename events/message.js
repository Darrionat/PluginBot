export default {
  ...
  async execute(client, message) {
    ...
    if (client.cooldowns.has(`${message.guild.id}-${message.author.id}`)) {
      if (
        ...
        return message.reply(
          `Please wait ${command.cooldown} second(s) before running that command again!`
        );
    }
    ...
    if (command.nsfw && !message.channel.nsfw)
      return message.reply(
        "That command is marked as NSFW. Please run it in an NSFW channel!"
      );
    ...
    if (command.args_required > 0 && args.length < command.args_required) {
      if (!command.args_usage)
        return message.reply(
          "You didn't provide enough arguments for that command!"
        );
      else
        return message.reply(
          `You didn't provide enough arguments for that command! Correct Usage: \`${client.config.prefix}${command.name} ${command.args_usage}\``
        );
    }
    ...
    if (command.user_permissions && command.user_permissions.length > 0) {
      if (!message.member.permissions.has(command.user_permissions))
        return message.reply(
          `You don't have permission to run that command! \`${command.user_permissions.join(
            "` `"
          )}\``
        );
    }
    ...
    if (command.bot_permissions && command.bot_permissions.length > 0) {
      if (!message.guild.members.me.permissions.has(command.bot_permissions))
        return message.reply(
          `I don't have correct permissions to run that command! \`${command.bot_permissions.join(
            "` `"
          )}\``
        );
    }
